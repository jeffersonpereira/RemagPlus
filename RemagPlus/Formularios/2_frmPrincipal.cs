...
namespace RemagPlus
{
    ...
    public partial class frmPrincipal : Form
    {
        ...
        private void configuraçãoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                remag_configuracao configura = new remag_configuracao();
                configura.remag = config.Remag;
                configura.sefip = config.Sefip;
                if (config.operacao == TipoOperacao.Adicionando)
                {
                    Crud<remag_configuracao>.New(configura);
                }
                else
                {
                    Crud<remag_configuracao>.Update(configura);
                }
                if (Crud<remag_configuracao>.SaveAll())
                {
                    MessageBox.Show(Mensagens.Salvo, Mensagens.Titulo, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                else
                {
                    MessageBox.Show("Não foi possível salvar o registro", Mensagens.Titulo, MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        {
            frmExclusaoIndividualizacao form = new frmExclusaoIndividualizacao();
            form.ShowDialog();
        }
